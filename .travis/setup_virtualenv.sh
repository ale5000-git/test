...
if [[ $VER == '3.1' ]]; then
  "virtualenv-$VER" -p "python$VER" --no-setuptools --no-pip --no-wheel "$HOME/virtualenv/python$VER" || exit 1
elif [[ $VER == '_2.4' ]]; then
...
elif [[ $TRAVIS_PYTHON_VERSION == '_2.5' ]]; then
  echo '*** Python - Updating Setuptools...'
  #easy_install setuptools==1.4.2 || exit 1
elif [[ $TRAVIS_PYTHON_VERSION == '_2.4' ]]; then
  echo '*** Python - Updating Setuptools...'
  #pip install setuptools==1.4.2 || exit 1
fi
